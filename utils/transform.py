...
class Augmentation:
    def __init__(
        ...
        self.weak_aug = transforms.Compose(
            [
                ...
                transforms.RandomApply(
                    [transforms.ColorJitter(0.8 * s, 0.8 * s, 0.4 * s, 0.2 * s)],
                    p=0.8),
                transforms.RandomGrayscale(p=0.2),
                transforms.RandomApply([GaussianBlur([.1, 2.])], p=blur),
                transforms.ToTensor(),
        ...
        self.val_aug = transforms.Compose(
            [
                # transforms.Resize(
                #     (val_size, val_size), interpolation=Image.BICUBIC
                # ),
                # transforms.CenterCrop(img_size),

                transforms.Resize(
                    (img_size, img_size), interpolation=Image.BICUBIC
                ),
...
def build_transform(is_train, args):
    if is_train:
        ...
        if args.dataset == "CIFAR-10":
            ...
            augmentation = Augmentation().val_aug
    return augmentation
